...
create table IF NOT EXISTS InstantCostElasticityPathway (ID int AUTO_INCREMENT PRIMARY KEY, monSeqID VARCHAR(200),  timestampID int, elasticityPathway OTHER, FOREIGN KEY (monSeqID) REFERENCES MonitoringSeq(ID),FOREIGN KEY (timestampID) REFERENCES Timestamp(ID) )
